...
public class getHead2 {
	...
	public static void main(String argv[]) {
		...
		try{//Inicio del código que puede generar excepciones
			...
			//Solicita al usuario el host, el puerto, y recurso del que obtener la cabecera
			System.out.println("Introduzca el nombre del servidor:  (ejemplo: www.uoc.edu)");
			host = in.readLine();
			System.out.println("Introduzca el numero de puerto:  (ejemplo: 80)");
			...
			sb.append("Host: " + host);
			sb.append(NL);
			sb.append("User-agent: ebalsa");
			sb.append(NL);
			mensaje=sb.toString();
			System.out.println(mensaje);//Muestra por la salida estandar el comando formado
			
			
			socket = new Socket(InetAddress.getByName(host), port); //Inicia una peticion de conexion al host, puerto 80
